...
def extract_data(date, datatype, volume, speed, thickness, coatingposition, trial, syringe):

    if datatype == 'flow':
        flow_data = pd.read_csv(date + 'data/' + datatype + '_' + date + '_' + volume + '_' + speed + '_' + thickness + '_' + syringe + '_' + 'syringe_' + coatingposition + trial + '_run_full'+ '.csv' )
        ...
        force_data = pd.read_csv(date + 'data/' + datatype + '_' + date + '_' + volume + '_' + speed + '_' + thickness + '_' + syringe + '_' + 'syringe_' + coatingposition + trial + '_run_full'+ '.csv' )
...
def plot_flow(date, datatype, volume, speed, thickness, coatingposition, trial, syringe):
    data, time_flow = extract_data(date, datatype, volume, speed, thickness, coatingposition, trial, syringe)
...
def plot_force(date, datatype, volume, speed, thickness, coatingposition, trial, syringe):
    data, time_force = extract_data(date, datatype, volume, speed, thickness, coatingposition, trial, syringe)
...
def plot_flow_force(date, volume, speed, thickness, coatingposition, trial, syringe):
    flow_time0, time_flow = extract_data(date, 'flow', volume, speed, thickness, coatingposition, trial, syringe)
    force_time0, time_force = extract_data(date, 'force_travel', volume, speed, thickness, coatingposition, trial, syringe)
...
def save_plot(date, datatype, volume, speed, thickness, coatingposition, trial, syringe):

    if datatype == 'flow':
        plot_flow(date, datatype, volume, speed, thickness, coatingposition, trial, syringe)
    elif datatype == 'force_travel':
        plot_force(date, datatype, volume, speed, thickness, coatingposition, trial, syringe)
    elif datatype == 'both':
        plot_flow_force(date, volume, speed, thickness, coatingposition, trial, syringe)
...
def compare_syringe(date, datatype, volume, speed, thickness, coatingposition, number):
    ...
    if datatype == 'flow':
        ...
        for n in range(number):
            k = n + 1
            syringe = input('Syringe' + str(k) + ' you want to compare:')
            data, time_flow = extract_data(date, datatype, volume, speed, thickness, coatingposition, trial, syringe)
            max_flow_index = data['flow'].idxmax()
            max_flow = data.loc[max_flow_index, 'flow']
            ...
            name += trial
            name += '_'
            fig = plt.plot(data['time'], data['flow'], marker='.', linestyle='none', label = syringe + 'syringe')
            fig = plt.plot(max_flow_time, max_flow, 'ro')
            fig = plt.text(max_flow_time, max_flow + 0.75, str(max_flow))
            fig = plt.plot(min_flow_time, min_flow, 'ro')
            fig = plt.text(min_flow_time, min_flow - 0.75, str(min_flow))
        ...
        plt.title('First trial of different syringes')
        plt.savefig(date + 'data/' + 'plot/' + 'compare_syringe_1st_trial' + str(number) + '_' + datatype + '_' + date + '_' + name + volume + '_' + speed + '.pdf')
    elif datatype == 'force_travel':
        max_force_list = []
        min_force_list = []
        max_travel_list = []
        ...
        for n in range(number):
            ...
            name += trial
            name += '_'
            fig = plt.plot(data['travel'], data['load'], marker='.', linestyle='none', label = syringe + 'syringe')
            fig = plt.plot(max_force_travel, max_force, 'ro')
            fig = plt.text(max_force_travel + 0.75, max_force , str(max_force))
            fig = plt.plot(min_force_travel, min_force, 'ro')
            fig = plt.text(min_force_travel + 0.75, min_force , str(min_force))
